...
public class AuthorVoExchanger extends VoEntityExchanger<AuthorVo, Author> {
    ...
    protected Author exchangeFrom(AuthorVo Vo) {
        return new Author(Vo.getName(), Vo.getBiography(), Vo.getBirthPlace(), Vo.getBirthdate());
    }

    @Override
    protected AuthorVo exchangeFrom(Author e) {
        return new AuthorVo(e.getName(), e.getBiography(), e.getBirthPlace(), e.getBirthdate());
    }
